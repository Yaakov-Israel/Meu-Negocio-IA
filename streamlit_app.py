...
try:
    ...
    st.info("Verifique as configurações do seu app no Streamlit Community Cloud e adicione a seção [firebase_config] com todas as chaves (apiKey, authDomain, etc.). O app não pode continuar sem isso.")
    st.stop()
except Exception as e: # Captura outras exceções potenciais ao acessar secrets
    st.error(f"🚨 ERRO CRÍTICO ao tentar acessar st.secrets['firebase_config']: {e}")
    st.stop()

# Inicializa o objeto de autenticação
# Para o streamlit-firebase-auth, geralmente apenas 'config' é obrigatório para começar.
# Outros parâmetros como cookie_name, key, cookie_expiry_days são para personalizar o cookie
# se o componente os suportar diretamente dessa forma (a documentação do componente é a melhor referência).
# Vamos manter simples por enquanto.
try:
    auth = st_auth.Authenticate(
        config=firebase_config,
        # Os próximos 3 parâmetros são opcionais e dependeriam se você quer
        # configurar um cookie customizado através desta biblioteca e se ela os usa
        # diretamente desta forma. Se não tiver [cookie_custom_auth] nos secrets, comente-os.
        # cookie_name=st.secrets.get("cookie_custom_auth", {}).get("name", "fb_cookie"), # Exemplo
        # key=st.secrets.get("cookie_custom_auth", {}).get("key", "fb_secret_key"),       # Exemplo
        # cookie_expiry_days=st.secrets.get("cookie_custom_auth", {}).get("expiry_days", 30) # Exemplo
    )
except Exception as e:
    st.error(f"🚨 ERRO ao inicializar o componente Authenticate do Firebase: {e}")
    st.info("Verifique se a biblioteca 'streamlit-firebase-auth' está no seu requirements.txt e se a configuração 'firebase_config' nos segredos está correta.")
    st.stop()

# --- FLUXO DE LOGIN ---
# O método login() do streamlit-firebase-auth geralmente renderiza o formulário
# e retorna informações do usuário ou None/False se não logado.
# Vamos verificar o estado da sessão que ele popula.
# Diferentes versões/implementações de componentes de autenticação podem variar um pouco.
# Vou usar o padrão que a maioria segue:
auth.login() # Renderiza o widget de login/registro

if not st.session_state.get("authentication_status"):
    # st.info("Por favor, faça login ou registre-se para continuar.") # Mensagem opcional
    st.stop() # Para a execução se não estiver autenticado

# --- SE AUTENTICADO, MOSTRA CONTEÚDO DO APP ---
st.sidebar.write(f"Bem-vindo, {st.session_state.get('username', 'Usuário Logado')}!") # Usa o username da session_state
if auth.logout("Logout", location="sidebar"): # Botão de logout
    st.experimental_rerun() # Re-executa para voltar à tela de login

# --- Carregar API Key e Configurar Modelo LLM (SÓ SE AUTENTICADO) ---
# Esta seção agora SÓ executa se o st.stop() acima não for atingido
GOOGLE_API_KEY = None
...
if not GOOGLE_API_KEY or not GOOGLE_API_KEY.strip():
    ...
    try:
        ...
        # st.sidebar.success("✅ Modelo LLM (Gemini) inicializado!") # Movido para depois da interface principal
    except Exception as e:
...
# --- RESTANTE DO SEU CÓDIGO (FUNÇÕES, CLASSE, INTERFACE PRINCIPAL) ---
# Cole todo o restante do seu código (a partir de "# --- FUNÇÕES AUXILIARES PARA MARKETING DIGITAL...") aqui.
# Certifique-se de que a inicialização do agente e a interface principal
# só ocorram se llm_model_instance não for None.

# Exemplo de como proteger a inicialização do agente e a UI principal:
if llm_model_instance:
    ...
    class AssistentePMEPro:
        def __init__(self, llm_passed_model):
            if llm_passed_model is None:
                ...
                st.stop() # Parar se o LLM não for passado
            self.llm = llm_passed_model
            # Inicializa memórias para cada chat no session_state para persistência
        ...
        def marketing_digital_guiado(self):
            ...
            with st.sidebar: 
                # Este if garante que o uploader só é mostrado se a seção de marketing estiver ativa
                # Isso evita que ele apareça em outras seções onde não é relevante
                # No entanto, para um uploader GERAL de marketing, ele pode ficar sempre visível
                # quando a seção de marketing_guiado está ativa.
                # st.subheader("📎 Suporte para Marketing") # Já temos o título da sidebar
                uploaded_marketing_files = st.file_uploader(
                    "Upload de arquivos de CONTEXTO para Marketing (opcional):", # Label mais claro
                    accept_multiple_files=True,
                    type=['png', 'jpg', 'jpeg', 'txt', 'md', 'pdf', 'csv', 'xlsx', 'docx', 'pptx'], # Removido vídeo para simplificar contexto
                    key="marketing_files_uploader_v15_final_CONTEXT" # Chave mais específica
                )
                ...
                # st.markdown("---") # Sidebar já tem um divisor

            main_action_key = "main_marketing_action_choice_v15_final" # Esta chave está OK
            main_action = st.radio(
                ...
                index=st.session_state.get(f"{main_action_key}_index", 0), # Persiste a seleção do radio
                key=main_action_key,
                on_change=lambda: st.session_state.update({f"{main_action_key}_index": list(opcoes_menu_marketing.values()).index(st.session_state[main_action_key]) if st.session_state[main_action_key] in opcoes_menu_marketing else 0})
            )
            st.markdown("---")
            
            opcoes_menu_marketing = { # Para usar no on_change do radio
            ...
            if main_action == "1 - Criar post para redes sociais ou e-mail":
                ...
                with st.form("post_creator_form_v15_final"):
                    ...
                    for i, (platform_name, platform_suffix) in enumerate(platforms_config_options.items()):
                        ...
                        if "E-mail Marketing" in platform_name and st.session_state.get(platform_key): 
                            # Mostra caption apenas se a opção de email correspondente estiver marcada
                ...
                if submit_button_pressed_post:
                    # A lógica para determinar actual_selected_platforms já foi feita com selected_platforms_post_ui
                    # Se select_all_post_checked for True, selected_platforms_post_ui conterá todas as plataformas.
                    # Se não, conterá apenas as marcadas.
        ...
        def conversar_plano_de_negocios(self, input_usuario): # Prompts completos devem ser revisados e inseridos
        ...
        def calcular_precos_interativo(self, input_usuario, descricao_imagem_contexto=None):
            ...
            # Ajuste no input para a cadeia, já que o prompt do sistema é bem detalhado.
    ...
    def exibir_chat_e_obter_input(area_chave, prompt_placeholder, funcao_conversa_agente, **kwargs_funcao_agente):
        chat_display_key = f"chat_display_{area_chave}"
        # Garante que a chave de display existe no session_state
    ...
    # Mensagem de sucesso do LLM movida para após a inicialização do agente, garantindo que tudo está ok
    ...
    URL_DO_SEU_LOGO_APP = "images/logo-pme-ia.png" # Tentativa de usar logo local do app (precisa estar no repo do app)
    # Tenta carregar a logo local, se falhar (ex: arquivo não existe), usa a URL externa como fallback
    try:
        # Verifica se o arquivo existe. Ajuste o caminho se a pasta 'images' não estiver na raiz do repo do app.
        # Para apps no Streamlit Cloud, o caminho é relativo à raiz do repositório.
        if os.path.exists(URL_DO_SEU_LOGO_APP):
            st.sidebar.image(URL_DO_SEU_LOGO_APP, width=150)
        else:
            # Fallback para a URL do Imgur se o arquivo local não for encontrado
        ...
        st.sidebar.image("https://i.imgur.com/7IIYxq1.png", width=150, caption="Logo (Fallback Erro)") # Fallback em caso de erro
    ...
    # Assegura que 'area_selecionada' existe e tem um valor válido antes de usá-lo para 'index'
    ...
    # Inicialização do session_state para o índice do radio
    ...
    area_selecionada_label = st.sidebar.radio(
        ...
        index=st.session_state[f'{radio_key_sidebar}_index'], # Usa o índice do session_state
    ...
    # Atualiza area_selecionada com base na seleção do radio
                ...
                if key_to_clear.startswith("post_v15_platform_") or \
                   ...
                   key_to_clear == "campaign_v15_select_all":
                    if st.session_state.get(key_to_clear) is not None:
                        del st.session_state[key_to_clear]
        st.rerun() # Força o rerun para atualizar a UI e a lógica de inicialização de chat
            ...
            for nome_menu_btn_pg, chave_secao_btn_pg in opcoes_menu.items():
                if chave_secao_btn_pg != "pagina_inicial":
                    ...
                    if col_para_botao_pg.button(button_label_pg, key=f"btn_goto_{chave_secao_btn_pg}_v15_final", use_container_width=True, help=f"Ir para {nome_menu_btn_pg}"):
                        ...
                        st.session_state[f'{radio_key_sidebar}_index'] = list(opcoes_menu.keys()).index(nome_menu_btn_pg) # Atualiza índice para o radio
                        st.rerun()
...
st.sidebar.markdown("---")
st.sidebar.info("Desenvolvido por Yaakov Israel com Gemini 2.5 pro") # Sua alteração mantida
