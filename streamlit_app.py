...
if user_is_authenticated:
    ...
    llm_init_exception = None # Para armazenar a exceção da inicialização do LLM

    if not GOOGLE_API_KEY or not GOOGLE_API_KEY.strip():
        st.error("🚨 ERRO: Chave API 'GOOGLE_API_KEY' não encontrada ou vazia nos Segredos do Streamlit.")
        st.stop() # App não pode funcionar sem a chave API
    else:
        try:
            ...
            # Não damos st.stop() aqui, para permitir que o logout funcione, 
            # mas a UI principal verificará se llm_model_instance é None.
    ...
    if llm_model_instance:
        ...
        def _marketing_handle_criar_post(uploaded_files_info, details_dict, selected_platforms_list, llm):
            ...
            with st.spinner("🤖 A IA está criando seu post... Aguarde!"):
                ...
                final_prompt = "\n\n".join(prompt_parts)
                # st.expander("🔍 Debug: Ver Prompt do Post").write(final_prompt) # Linha de debug para post, se necessário
        ...
        class AssistentePMEPro:
            def __init__(self, llm_passed_model):
                ...
                # Usando APP_KEY_SUFFIX para chaves de memória, como no seu código
            ...
            def _criar_cadeia_conversacional(self, system_message_content, memoria_especifica, memory_key_placeholder_base="historico_chat"): # Renomeado placeholder_base
            ...
            def marketing_digital_guiado(self):
                ...
                # A lógica do file_uploader da sidebar (conforme seu código original)
                with st.sidebar: 
                    # Este expander pode ser removido se não for desejado na sidebar,
                    # ou o file_uploader pode ser movido para o corpo principal da seção de marketing se preferir.
                    # Vou manter como no seu código de 852 linhas por enquanto.
                    ...
                    with sidebar_marketing_expander:
                        ...
                        if uploaded_marketing_files_sidebar:
                            ...
                            for up_file in uploaded_marketing_files_sidebar:
                ...
                if radio_index_key_mkt not in st.session_state:
                    st.session_state[radio_index_key_mkt] = 0

                # Corrigindo a lógica do on_change para o radio de marketing
                current_main_action_value = st.session_state.get(main_action_key, opcoes_radio_marketing[0])
                try:
                    current_main_action_index = opcoes_radio_marketing.index(current_main_action_value)
                    if st.session_state.get(radio_index_key_mkt) != current_main_action_index :
                         st.session_state[radio_index_key_mkt] = current_main_action_index
                except ValueError: # Caso o valor em session_state não seja uma opção válida
                    st.session_state[radio_index_key_mkt] = 0
                ...
                main_action = st.radio(
                    ...
                    index=st.session_state[radio_index_key_mkt], 
                    key=main_action_key
                    # on_change removido para simplificar, o estado do radio já é gerenciado
                )
                # Atualizar o índice após a renderização do radio, se necessário para lógica futura
                # ou se o on_change fosse mantido, ele precisaria de uma função de callback.
                # A forma mais simples é ler st.session_state[main_action_key] diretamente.
                ...
                if main_action == "1 - Criar post para redes sociais ou e-mail":
                    ...
                    with st.form(f"post_creator_form{APP_KEY_SUFFIX}"):
                        ...
                        select_all_post_checked_val = st.session_state.get(key_select_all_post, False) # Ler valor atual
                        ...
                        selected_platforms_post_keys_map = {}
                        for i, (platform_name, platform_suffix) in enumerate(platforms_config_options.items()):
                            ...
                            selected_platforms_post_keys_map[platform_name] = platform_key
                            with cols_post[col_index]:
                                st.checkbox(platform_name, key=platform_key, value=(select_all_post_checked if st.session_state.get(key_select_all_post) else st.session_state.get(platform_key, False) ))
                    ...
                    if submit_button_pressed_post:
                        ...
                        if st.session_state.get(key_select_all_post, False):
                            ...
                            for p_name, p_key in selected_platforms_post_keys_map.items():
                    ...
                    with st.form(f"campaign_creator_form{APP_KEY_SUFFIX}"):
                        campaign_name = st.text_input("Nome da Campanha:", key=f"campaign_name_ui{APP_KEY_SUFFIX}") # Chave UI única
                        st.subheader(" Plataformas Desejadas:")
                        ...
                        selected_platforms_camp_keys_map = {}
                        for i, (platform_name, platform_suffix) in enumerate(platforms_config_options.items()):
                            col_index = i % 2
                            platform_key = f"campaign_platform_{platform_suffix}{APP_KEY_SUFFIX}"
                            selected_platforms_camp_keys_map[platform_name] = platform_key
                            with cols_camp[col_index]:
                                st.checkbox(platform_name, key=platform_key, value=(select_all_camp_checked if st.session_state.get(key_select_all_camp) else st.session_state.get(platform_key, False)))
                        
                        campaign_details_obj = _marketing_get_objective_details(f"campaign_creator_details{APP_KEY_SUFFIX}", "campanha")
                        campaign_duration = st.text_input("Duração Estimada:", key=f"campaign_duration_ui{APP_KEY_SUFFIX}") # Chave UI única
                        campaign_budget_approx = st.text_input("Orçamento Aproximado (opcional):", key=f"campaign_budget_ui{APP_KEY_SUFFIX}") # Chave UI única
                        specific_kpis = st.text_area("KPIs mais importantes:", key=f"campaign_kpis_ui{APP_KEY_SUFFIX}") # Chave UI única
                ...
                # ... (Restante dos blocos elif para outras ações de marketing, adaptando chaves para APP_KEY_SUFFIX)
            ...
            # Esta é a estrutura do seu código, com memory_key_placeholder ajustado
            ...
            def calcular_precos_interativo(self, input_usuario, descricao_imagem_contexto=None):
                ...
                # O input do usuário já é o 'input_usuario', a pergunta adicional é desnecessária aqui
                resposta_ai_obj = cadeia.invoke({"input_usuario": input_usuario}) 
                return resposta_ai_obj['text'] if isinstance(resposta_ai_obj, dict) and 'text' in resposta_ai_obj else str(resposta_ai_obj)
            ...
            def gerar_ideias_para_negocios(self, input_usuario, contexto_arquivos=None):
                ...
                # O input do usuário já é o 'input_usuario'
                resposta_ai_obj = cadeia.invoke({"input_usuario": input_usuario})
                return resposta_ai_obj['text'] if isinstance(resposta_ai_obj, dict) and 'text' in resposta_ai_obj else str(resposta_ai_obj)
        ...
        def inicializar_ou_resetar_chat(area_chave, mensagem_inicial_ia, memoria_agente_instancia):
            ...
            if memoria_agente_instancia:
                ...
                # Adicionando a mensagem inicial à memória Langchain de forma robusta
                if hasattr(memoria_agente_instancia, 'chat_memory') and hasattr(memoria_agente_instancia.chat_memory, 'messages'):
                    memoria_agente_instancia.chat_memory.messages.clear() # Limpa primeiro se for lista
                    memoria_agente_instancia.chat_memory.add_ai_message(mensagem_inicial_ia) # Método preferido se existir
                elif hasattr(memoria_agente_instancia.chat_memory, 'add_ai_message'): # Se não for lista mas tiver add_ai_message
                     memoria_agente_instancia.chat_memory.add_ai_message(mensagem_inicial_ia)
            
            # Limpando chaves específicas de upload
            if area_chave == "calculo_precos": 
                st.session_state.pop(f'last_uploaded_image_info_calculo_precos{APP_KEY_SUFFIX}', None) # Chave corrigida
                st.session_state.pop(f'processed_image_id_calculo_precos{APP_KEY_SUFFIX}', None) # Chave corrigida
                st.session_state.pop(f'user_input_processed_calculo_precos{APP_KEY_SUFFIX}', None) # Chave corrigida
            elif area_chave == "gerador_ideias": 
                st.session_state.pop(f'uploaded_file_info_gerador_ideias_for_prompt{APP_KEY_SUFFIX}', None) # Chave corrigida
                st.session_state.pop(f'processed_file_id_gerador_ideias{APP_KEY_SUFFIX}', None) # Chave corrigida
                st.session_state.pop(f'user_input_processed_gerador_ideias{APP_KEY_SUFFIX}', None) # Chave corrigida
        ...
        def _handle_chat_with_image(area_chave, prompt_placeholder, funcao_conversa_agente, uploaded_image_obj): 
            ...
            if uploaded_image_obj is not None:
                # Usar uploaded_image_obj.file_id para garantir unicidade
            ...
            if user_input_processed_key in st.session_state and st.session_state[user_input_processed_key]:
                # Limpa a informação da imagem da sessão APÓS o input do usuário ter sido processado pela IA
                # para que não seja reenviada no próximo turno se o usuário não carregar nova imagem.
                st.session_state.pop(last_uploaded_info_key, None) 
        ...
        def _handle_chat_with_files(area_chave, prompt_placeholder, funcao_conversa_agente, uploaded_files_objs):
            ...
            if uploaded_files_objs:
                # Criar uma assinatura única para a lista de arquivos atual
            ...
            if user_input_processed_key_handle in st.session_state and st.session_state[user_input_processed_key_handle]:
                st.session_state.pop(uploaded_info_key_handle, None) # Limpa após o uso
                st.session_state[user_input_processed_key_handle] = False

        # --- Instanciação do Agente ---
        # (Como no seu código de 852 linhas, verificando também se o llm mudou)
        ...
        if st.sidebar.button("Logout", key=f"main_app_logout{APP_KEY_SUFFIX}"): 
            ...
            # Adicionar chaves de session_state específicas do app que usam APP_KEY_SUFFIX ou prefixos conhecidos
            ...
            for key_to_clear in set(keys_to_clear_logout): # Usar set para evitar duplicatas
        ...
        # Usando uma chave única para o radio principal da sidebar
        ...
        # Obtendo o índice atual para o radio button
        try:
            current_nav_radio_index = list(opcoes_menu.keys()).index(st.session_state.area_selecionada)
        except ValueError:
            current_nav_radio_index = 0 # Padrão para "Página Inicial"
            st.session_state.area_selecionada = list(opcoes_menu.keys())[0]
        
        # O on_change não é estritamente necessário se lermos o valor do radio diretamente
        # mas se usado, a função de callback deve apenas atualizar o st.session_state.area_selecionada
        # e o rerun fará o resto.
        # Para simplificar e alinhar com a estrutura do seu código original que não usava on_change no radio principal:
        area_selecionada_label = st.sidebar.radio(
            "Como posso te ajudar hoje?", 
            options=list(opcoes_menu.keys()), 
            key=radio_key_sidebar_main_nav_app, # Chave única
            index=current_nav_radio_index
        )

        if area_selecionada_label != st.session_state.area_selecionada:
            st.session_state.area_selecionada = area_selecionada_label
            # Limpeza de estados específicos ao mudar de seção (ex: marketing)
            if area_selecionada_label != "Marketing Digital com IA (Guia)":
                keys_to_clear_mkt_nav_app = [k for k in st.session_state if (k.startswith("generated_") and k.endswith(f"_new{APP_KEY_SUFFIX}")) or \
                                                                        (f"post{APP_KEY_SUFFIX}" in k) or (f"campaign{APP_KEY_SUFFIX}" in k) or \
                                                                        k == f"main_marketing_action_choice{APP_KEY_SUFFIX}_index" ] # Limpar o índice do radio de marketing
                for key_clear_mkt_app in keys_to_clear_mkt_nav_app:
                    st.session_state.pop(key_clear_mkt_app, None)
            st.rerun() 

        current_section_key = opcoes_menu.get(st.session_state.area_selecionada)
        
        # Inicializar chats quando a seção é selecionada (lógica do seu código)
        if current_section_key not in ["pagina_inicial", "marketing_guiado"]:
            ...
            if needs_chat_reset_main_app:
                ...
                if msg_inicial_nav_app and memoria_agente_nav_app is not None:
                    inicializar_ou_resetar_chat(current_section_key, msg_inicial_nav_app, memoria_agente_nav_app)
        ...
        if current_section_key == "pagina_inicial":
            ...
            if num_botoes_funcionais > 0 :
                num_cols_render = min(num_botoes_funcionais, 3) # Mantendo 3 colunas para os botões
                ...
                for nome_menu_btn_pg, chave_secao_btn_pg in opcoes_menu.items():
                    if chave_secao_btn_pg != "pagina_inicial":
                        ...
                        if col_para_botao_pg.button(button_label_pg, key=f"btn_goto_{chave_secao_btn_pg}{APP_KEY_SUFFIX}", use_container_width=True, help=f"Ir para {nome_menu_btn_pg}"):
                            st.session_state.area_selecionada = nome_menu_btn_pg
                            try: 
                                st.session_state[radio_index_key_main_nav] = list(opcoes_menu.keys()).index(nome_menu_btn_pg)
            ...
            uploaded_image_preco = st.file_uploader("Envie uma imagem do produto (opcional):", type=["png", "jpg", "jpeg"], key=f"preco_img_uploader{APP_KEY_SUFFIX}") # Usando APP_KEY_SUFFIX
            _handle_chat_with_image("calculo_precos", "Descreva o produto/serviço, custos, etc.", agente.calcular_precos_interativo, uploaded_image_preco)
            ...
            uploaded_files_ideias_app_ui = st.file_uploader("Envie arquivos de contexto (opcional - .txt, .png, .jpg):", type=["txt", "png", "jpg", "jpeg"], accept_multiple_files=True, key=f"ideias_file_uploader{APP_KEY_SUFFIX}") # Usando APP_KEY_SUFFIX
        ...
        if llm_init_exception: 
            st.exception(llm_init_exception) # Mostra a exceção da inicialização do LLM


# --- Seção de Login/Registro (executada se user_is_authenticated for False) ---
# (Esta parte é do nosso esqueleto de autenticação funcional)
else: 
    st.session_state.pop('auth_error_shown', None) 
    st.title("🔑 Bem-vindo ao Assistente PME Pro") 

    st.sidebar.subheader("Login / Registro")
    auth_action_choice_key_else_final = f"app_auth_action_choice{APP_KEY_SUFFIX}_else_final" # Chave única
    ...
    if auth_action_choice == "Login":
        with st.sidebar.form(f"app_login_form{APP_KEY_SUFFIX}_else_final"): # Chave única
            ...
            if login_button_clicked:
                if login_email and login_password and pb_auth_client:
                    try:
                        ...
                        error_message_login_final = "Erro no login. Verifique suas credenciais."
                        try: 
                            error_details_str_final = e_login_final.args[0] if len(e_login_final.args) > 0 else "{}"
                            error_data_final = json.loads(error_details_str_final.replace("'", "\""))
                            api_error_message_final = error_data_final.get('error', {}).get('message', '')
                            if "INVALID_LOGIN_CREDENTIALS" in api_error_message_final or "EMAIL_NOT_FOUND" in api_error_message_final or "INVALID_PASSWORD" in api_error_message_final or "USER_DISABLED" in api_error_message_final or "INVALID_EMAIL" in api_error_message_final:
                                error_message_login_final = "Email ou senha inválidos, ou usuário desabilitado."
                            elif api_error_message_final: error_message_login_final = f"Erro no login: {api_error_message_final}"
                        except: pass 
                        st.sidebar.error(error_message_login_final)
        ...
        with st.sidebar.form(f"app_register_form{APP_KEY_SUFFIX}_else_final"): # Chave única
            ...
            if submit_register:
                if reg_email and reg_password and pb_auth_client:
                    try:
                        ...
                        except Exception as verify_email_error_final: 
                           st.sidebar.caption(f"Nota: Não foi possível enviar email de verificação: {verify_email_error_final}")
                    except Exception as e_register_final:
                        error_message_register_final = "Erro no registro."
                        try:
                            error_details_str_reg_final = e_register_final.args[0] if len(e_register_final.args) > 0 else "{}"
                            error_data_reg_final = json.loads(error_details_str_reg_final.replace("'", "\""))
                            api_error_message_reg_final = error_data_reg_final.get('error', {}).get('message', '')
                            if "EMAIL_EXISTS" in api_error_message_reg_final:
                                error_message_register_final = "Este email já está registrado. Tente fazer login."
                            elif api_error_message_reg_final:
                                error_message_register_final = f"Erro no registro: {api_error_message_reg_final}"
                        except: 
                             error_message_register_final = f"Erro no registro: {str(e_register_final)}"
                        st.sidebar.error(error_message_register_final)
                elif not pb_auth_client: st.sidebar.error("Cliente Firebase Auth não inicializado.")
