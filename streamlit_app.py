...
if st.session_state['authentication_status']:
    ...
    authenticator.logout("Logout", "sidebar", key="logout_button_stauth_final_v4")
    ...
    def _marketing_get_objective_details(section_key, type_of_creation="post/campanha"):
        key_suffix = f"_{section_key}_mkt_obj_v10_stauth"
    ...
    def _marketing_display_output_options(generated_content, section_key, file_name_prefix="conteudo_gerado"):
        key_suffix = f"_{section_key}_output_v10_stauth"
    ...
    def _marketing_handle_criar_post(uploaded_files_info, details_dict, selected_platforms_list, llm):
        ...
        with st.spinner("🤖 Criando seu post..."):
            ...
            final_prompt = "\n\n".join(prompt_parts)
            try: st.session_state.generated_post_content_stauth_v5 = llm.invoke([HumanMessage(content=final_prompt)]).content
    ...
    def _marketing_handle_criar_campanha(uploaded_files_info, details_dict, campaign_specifics, selected_platforms_list, llm):
        ...
        with st.spinner("🧠 Elaborando plano de campanha..."):
            ...
            if uploaded_files_info: prompt_parts.append(f"**Arquivos de Suporte:** {', '.join([f['name'] for f in uploaded_files_info])}.")
            final_prompt = "\n\n".join(prompt_parts)
            try: st.session_state.generated_campaign_content_stauth_v5 = llm.invoke([HumanMessage(content=final_prompt)]).content
    ...
    def _marketing_handle_gerar_texto_ativo(campaign_plan_context, asset_description, asset_objective, llm):
        ...
        with st.spinner("✍️ IA escrevendo..."):
            ...
            try: st.session_state.current_generated_asset_text_stauth_v5 = llm.invoke([HumanMessage(content=prompt)]).content
            except Exception as e: st.error(f"Erro: {e}"); st.session_state.current_generated_asset_text_stauth_v5 = "Erro."
    ...
    def _marketing_handle_criar_landing_page(uploaded_files_info, lp_details, llm):
        ...
        with st.spinner("🎨 Desenhando landing page..."):
            ...
            if uploaded_files_info: prompt_parts.append(f"**Arquivos de Suporte:** {', '.join([f['name'] for f in uploaded_files_info])}.")
            final_prompt = "\n\n".join(prompt_parts); st.session_state.generated_lp_content_stauth_v5 = llm.invoke([HumanMessage(content=final_prompt)]).content
    ...
    def _marketing_handle_criar_site(uploaded_files_info, site_details, llm):
        ...
        with st.spinner("🛠️ Arquitetando site..."):
            ...
            if uploaded_files_info: prompt_parts.append(f"**Arquivos de Suporte:** {', '.join([f['name'] for f in uploaded_files_info])}.")
            final_prompt = "\n\n".join(prompt_parts); st.session_state.generated_site_content_stauth_v5 = llm.invoke([HumanMessage(content=final_prompt)]).content
    ...
    def _marketing_handle_encontre_cliente(uploaded_files_info, client_details, llm):
        ...
        with st.spinner("🕵️ Investigando público-alvo..."):
            ...
            if uploaded_files_info: prompt_parts.append(f"**Arquivos de Suporte:** {', '.join([f['name'] for f in uploaded_files_info])}.")
            final_prompt = "\n\n".join(prompt_parts); st.session_state.generated_client_analysis_stauth_v5 = llm.invoke([HumanMessage(content=final_prompt)]).content
    ...
    def _marketing_handle_conheca_concorrencia(uploaded_files_info, competitor_details, llm):
        ...
        with st.spinner("🔬 Analisando concorrência..."):
            ...
            if uploaded_files_info: prompt_parts.append(f"**Arquivos de Suporte:** {', '.join([f['name'] for f in uploaded_files_info])}.")
            final_prompt = "\n\n".join(prompt_parts); st.session_state.generated_competitor_analysis_stauth_v5 = llm.invoke([HumanMessage(content=final_prompt)]).content

    class AssistentePMEPro: # Esta classe deve estar corretamente indentada
        def __init__(self, llm_passed_model):
            if llm_passed_model is None: 
                st.error("Erro: Modelo LLM não inicializado corretamente.")
                st.stop()
            self.llm = llm_passed_model
            self.memoria_plano_negocios = ConversationBufferMemory(memory_key="chat_hist_plano_v10stauth", return_messages=True)
            self.memoria_calculo_precos = ConversationBufferMemory(memory_key="chat_hist_precos_v10stauth", return_messages=True)
            self.memoria_gerador_ideias = ConversationBufferMemory(memory_key="chat_hist_ideias_v10stauth", return_messages=True)

        def _criar_cadeia_conversacional(self, system_message_content, memoria_especifica, memory_key_placeholder="chat_hist_v10stauth"):
        ...
        def marketing_digital_guiado(self):
            ...
            if main_action != "mkt_selecione": st.markdown("---") # Divisor visual se uma ação for selecionada
            ...
            if main_action == "mkt_post":
                ...
                if 'generated_post_content_stauth_v5' in st.session_state: 
                    _marketing_display_output_options(st.session_state.generated_post_content_stauth_v5, f"{base_key_mkt}_post_out", "post_ia")

            elif main_action == "mkt_campanha":
                st.subheader("🌍 Planejador de Campanhas")
                with st.form(f"{base_key_mkt}_campaign_form"):
                    camp_name = st.text_input("Nome da Campanha:", key=f"{base_key_mkt}_camp_name_v3")
                    st.subheader("Plataformas:")
                    cols_c = st.columns(min(len(platforms_cfg), 4))
                    platform_states_camp = {name: cols_c[i % len(cols_c)].checkbox(name, key=f"{base_key_mkt}_camp_p_{suf}_v3") for i, (name, suf) in enumerate(platforms_cfg.items())}
                    camp_details_obj = _marketing_get_objective_details(f"{base_key_mkt}_camp_details_v3", "campanha")
                    camp_duration = st.text_input("Duração:", key=f"{base_key_mkt}_camp_duration_v3")
                    camp_budget = st.text_input("Orçamento (opcional):", key=f"{base_key_mkt}_camp_budget_v3")
                    camp_kpis = st.text_area("KPIs:", key=f"{base_key_mkt}_camp_kpis_v3")
                ...
                if 'generated_campaign_content_stauth_v5' in st.session_state:
                    _marketing_display_output_options(st.session_state.generated_campaign_content_stauth_v5, f"{base_key_mkt}_camp_out", "campanha_ia")
                    # Botão para criar ativos fora do form principal da campanha
                    if st.button("🚀 Criar Ativos da Campanha", key=f"{base_key_mkt}_btn_create_assets_v3"):
                        st.session_state.creating_campaign_assets_stauth_v5 = True
                        st.session_state.current_campaign_plan_context_stauth_v5 = st.session_state.generated_campaign_content_stauth_v5
                        st.session_state.campaign_assets_list_stauth_v5 = [] 
                        st.rerun()
                if st.session_state.get("creating_campaign_assets_stauth_v5"):
                    st.markdown("---"); st.subheader("🛠️ Criador de Ativos para Campanha")
                    st.info(st.session_state.get('current_campaign_plan_context_stauth_v5', "Contexto não carregado."))
                    asset_form_key = f"{base_key_mkt}_asset_creator_form_v3"
                    with st.form(asset_form_key, clear_on_submit=True):
                        ...
                        if st.session_state.get('current_generated_asset_text_stauth_v5'): st.text_area("Texto Gerado:", value=st.session_state.current_generated_asset_text_stauth_v5, height=100, disabled=True, key=f"{asset_form_key}_text_display")
                        submit_add_asset = st.form_submit_button("➕ Adicionar Ativo à Lista")
                        if submit_generate_text_asset:
                            if asset_desc and asset_obj: _marketing_handle_gerar_texto_ativo(st.session_state.get('current_campaign_plan_context_stauth_v5'), asset_desc, asset_obj, self.llm); st.rerun()
                            else: st.warning("Preencha Descrição e Objetivo.")
                        if submit_add_asset:
                            if asset_desc:
                                new_asset = {"descricao": asset_desc, "objetivo": asset_obj, "texto_gerado": st.session_state.get('current_generated_asset_text_stauth_v5', ""),"imagem_carregada": img_asset.name if img_asset else None, "video_carregado": vid_asset.name if vid_asset else None}
                                st.session_state.campaign_assets_list_stauth_v5.append(new_asset)
                                st.success(f"Ativo '{asset_desc}' adicionado!"); st.session_state.current_generated_asset_text_stauth_v5 = ""; st.rerun()
                            else: st.warning("Adicione uma descrição.")
                    # Botões de ideias fora do form
                    c1c,c2c = st.columns(2)
                    if c1c.button("💡 Ideias de Imagem (Campanha)", key=f"{base_key_mkt}_btn_img_ideas_asset_outside_v5"): st.info("Em desenvolvimento.")
                    if c2c.button("💡 Ideias de Vídeo (Campanha)", key=f"{base_key_mkt}_btn_vid_ideas_asset_outside_v5"): st.info("Em desenvolvimento.")
                    if st.session_state.get('campaign_assets_list_stauth_v5', []):
                        st.markdown("---"); st.subheader("📦 Ativos Adicionados:")
                        for i, asset in enumerate(st.session_state.campaign_assets_list_stauth_v5):
                            with st.expander(f"Ativo {i+1}: {asset['descricao']}"): st.write(asset)
                    if st.button("🏁 Concluir Criação de Ativos", key=f"{base_key_mkt}_btn_finish_assets_v5"):
                        st.session_state.creating_campaign_assets_stauth_v5 = False; st.success("Criação de ativos concluída!"); st.balloons(); st.rerun()
                ...
                if 'generated_lp_content_stauth_v5' in st.session_state: _marketing_display_output_options(st.session_state.generated_lp_content_stauth_v5, f"{base_key_mkt}_lp_out", "lp_ia")
                ...
                with st.form(f"{base_key_mkt}_site_form"):
                    ...
                    site_details["business_type"] = st.text_input("Tipo do seu Negócio/Empresa:", key=f"{base_key_mkt}_site_biz_type_v2")
                    site_details["essential_pages"] = st.text_area("Páginas Essenciais:", key=f"{base_key_mkt}_site_pages_v2")
                ...
                if 'generated_site_content_stauth_v5' in st.session_state: _marketing_display_output_options(st.session_state.generated_site_content_stauth_v5, f"{base_key_mkt}_site_out", "site_ia")

            elif main_action == "mkt_cliente":
                st.subheader("🎯 Decodificador de Clientes")
                with st.form(f"{base_key_mkt}_client_form"):
                    client_details = {}
                    client_details["product_campaign"] = st.text_area("Produto/serviço/campanha:", key=f"{base_key_mkt}_fc_campaign_v2")
                    client_details["location"] = st.text_input("Localização (opcional):", key=f"{base_key_mkt}_fc_location_v2")
                    client_details["budget"] = st.text_input("Verba para marketing (opcional):", key=f"{base_key_mkt}_fc_budget_v2")
                    client_details["age_gender"] = st.text_input("Faixa etária e/ou gênero principal:", key=f"{base_key_mkt}_fc_age_gender_v2")
                    client_details["interests"] = st.text_area("Interesses, hobbies, dores do público:", key=f"{base_key_mkt}_fc_interests_v2")
                    client_details["current_channels"] = st.text_area("Canais onde já tentou alcançar:", key=f"{base_key_mkt}_fc_channels_v2")
                    client_details["deep_research"] = st.checkbox("Habilitar 'Deep Research'", value=True, key=f"{base_key_mkt}_fc_deep_v2")
                ...
                if 'generated_client_analysis_stauth_v5' in st.session_state: _marketing_display_output_options(st.session_state.generated_client_analysis_stauth_v5, f"{base_key_mkt}_client_out", "cliente_ia")

            elif main_action == "mkt_concorrencia":
                st.subheader("🧐 Radar da Concorrência")
                with st.form(f"{base_key_mkt}_competitor_form"):
                    competitor_details = {}
                    competitor_details["your_business"] = st.text_area("Descreva seu negócio/produto:", key=f"{base_key_mkt}_ca_your_biz_v2")
                    competitor_details["competitors_list"] = st.text_area("Liste seus concorrentes:", key=f"{base_key_mkt}_ca_competitors_v2")
                    competitor_details["aspects_to_analyze"] = st.multiselect( "Aspectos a analisar:", ["Presença Online", "Conteúdo", "Comunicação", "Pontos Fortes", "Pontos Fracos", "Preços"], default=["Presença Online", "Pontos Fortes"], key=f"{base_key_mkt}_ca_aspects_v2")
                ...
                if 'generated_competitor_analysis_stauth_v5' in st.session_state: _marketing_display_output_options(st.session_state.generated_competitor_analysis_stauth_v5, f"{base_key_mkt}_competitor_out", "concorrencia_ia")

            elif main_action == "mkt_selecione": 
                st.info("Escolha uma ferramenta de marketing acima para começar.")

        # --- Funções de Chat (corrigidas para estarem dentro da classe) ---
    ...
    # --- Funções Utilitárias de Chat (fora da classe AssistentePMEPro) ---
    def inicializar_ou_resetar_chat(area_chave, mensagem_inicial_ia, memoria_agente_instancia):
        chat_display_key = f"chat_display_{area_chave}_v10stauth"
        ...
        if area_chave == "calculo_precos": 
            st.session_state.pop(f'last_uploaded_image_info_pricing_v10stauth', None)
            st.session_state.pop(f'processed_image_id_pricing_v10stauth', None)
        elif area_chave == "gerador_ideias": 
            st.session_state.pop(f'uploaded_file_info_ideias_for_prompt_v10stauth', None)
            st.session_state.pop(f'processed_file_id_ideias_v10stauth', None)

    def exibir_chat_e_obter_input(area_chave, prompt_placeholder, funcao_conversa_agente, **kwargs_funcao_agente):
        chat_display_key = f"chat_display_{area_chave}_v10stauth"; chat_input_key = f"chat_input_{area_chave}_v10stauth"
    ...
    if llm_model_instance:
        agente_key = "agente_pme_v10stauth"
        ...
        current_area_key_main = "area_selecionada_v10stauth"

        if current_area_key_main not in st.session_state: 
            st.session_state[current_area_key_main] = "Página Inicial"

        for _, chave_secao_init_loop_main in opcoes_menu.items():
            if chave_secao_init_loop_main not in ["marketing_guiado", "pagina_inicial"]:
                chat_key_init_loop_main = f"chat_display_{chave_secao_init_loop_main}_v10stauth"
                if chat_key_init_loop_main not in st.session_state:
                    st.session_state[chat_key_init_loop_main] = []

        previous_area_key_main_nav_loop_main = "previous_area_selec_v10stauth_nav"
        if previous_area_key_main_nav_loop_main not in st.session_state: 
            st.session_state[previous_area_key_main_nav_loop_main] = None

        area_selecionada_label = st.sidebar.radio(
            "Navegação Principal:", 
            options=list(opcoes_menu.keys()), 
            key='sidebar_select_v10stauth', 
        ...
        if current_section_key_val not in ["pagina_inicial", "marketing_guiado"]:
            if st.session_state[current_area_key_main] != st.session_state.get(previous_area_key_main_nav_loop_main):
                chat_display_key_nav = f"chat_display_{current_section_key_val}_v10stauth"
                ...
                st.session_state[previous_area_key_main_nav_loop_main] = st.session_state[current_area_key_main]
        ...
        if current_section_key_val == "pagina_inicial":
            ...
            if st.sidebar.button("Reiniciar Plano", key="btn_reset_plano_v10stauth"): inicializar_ou_resetar_chat(current_section_key_val, "Plano de negócios reiniciado.", agente.memoria_plano_negocios); st.rerun()
        elif current_section_key_val == "calculo_precos":
            st.header("💲 Cálculo de Preços Inteligente")
            exibir_chat_e_obter_input(current_section_key_val, "Detalhes do produto/serviço...", agente.calcular_precos_interativo)
            if st.sidebar.button("Reiniciar Cálculo", key="btn_reset_precos_v10stauth"): inicializar_ou_resetar_chat(current_section_key_val, "Novo cálculo. Descreva o item.", agente.memoria_calculo_precos); st.rerun()
        elif current_section_key_val == "gerador_ideias":
            st.header("💡 Gerador de Ideias para Negócios")
            exibir_chat_e_obter_input(current_section_key_val, "Qual seu desafio ou área de interesse?", agente.gerar_ideias_para_negocios)
            if st.sidebar.button("Nova Sessão de Ideias", key="btn_reset_ideias_v10stauth"): inicializar_ou_resetar_chat(current_section_key_val, "Pronto para novas ideias!", agente.memoria_gerador_ideias); st.rerun()
    else:
        st.error("Modelo de Linguagem (LLM) não pôde ser carregado.")

elif st.session_state['authentication_status'] is False:
    # st.error('Nome de usuário/senha incorretos.') # Mensagem já gerenciada pelo authenticator.login()
    pass 
elif st.session_state['authentication_status'] is None:
    URL_DO_SEU_LOGO_LOGIN = "https://i.imgur.com/7IIYxq1.png"
    if URL_DO_SEU_LOGO_LOGIN: 
        cols_login_img = st.columns([1,2,1]) # Colunas para centralizar a imagem
        with cols_login_img[1]:
    ...
    # A mensagem de "Por favor, insira..." já é gerenciada por authenticator.login()
    # que é chamado dentro de authentication_flow_stauth, que por sua vez é chamado
    # quando st.session_state['authentication_status'] is None (ou False).
